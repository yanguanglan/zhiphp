...
;(function($){
    $.ZhiPHP.topic = {
        ...
        init: function(options){
            options && $.extend($.ZhiPHP.topic.settings, options);
            ...
            $.ZhiPHP.topic.piczoom();
            $.ZhiPHP.topic.delete();
            $.ZhiPHP.topic.forward();
            $.ZhiPHP.topic.comment();
            $.ZhiPHP.topic.comment_sub();
            $.ZhiPHP.topic.comment_reply();
            $.ZhiPHP.topic.comment_del();
            $.ZhiPHP.topic.comment_me_reply();
            $.ZhiPHP.topic.comment_me_reply_sub();
        },
        //点击缩略查看详细
        piczoom: function(){
            var s = $.ZhiPHP.topic.settings;
                ...
                if(self.siblings('.J_src').html()){
                    ...
                    $.getJSON(PINER.root + '?m=space&a=feed_src', {src_id:src_id, src_type:src_type}, function(result){
                        if(result.status == 1){
                            ...
                            $.ZhiPHP.ui.decode_img(self.siblings('.J_src'));
                        }else{
        ...
        forward_form: function(form){
            form.ajaxForm({
                beforeSubmit: function(){
                    ...
                    if(content == ''){
                        $.ZhiPHP.tip({content:lang.please_input+lang.forward_content, icon:'error'});
                        return !1;
                    }
                },
            ...
            $(s.topic_unit).find('.J_cmt').toggle(
                function(){
                    ...
                    if(unit.find(s.cmt_pub)[0]){
                        ...
                        $.ZhiPHP.topic.comment_list(unit);
                    }
                },
                function(){
