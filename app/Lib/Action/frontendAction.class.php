...
class frontendAction extends baseAction {
    ...
    function _assign_common(){
        ...
        $this->assign("all_post_cate_list",$this->post_cate_mod->where("status=1 and pid<2")->order("ordid desc")->select()); 
        $about_content=msubstr(strip_tags($this->article_page_mod->where("cate_id=2")->getField("info")),350);
    ...
    */
    ...
    protected function _parse_post($list){
        foreach($list as $key=>$val){
            ...
            $list[$key]['tag_list']=$this->tag_mod->where("id in(select tag_id from ".table("post_tag")." where post_id=$val[id])")->select();                      
        }        
        return $list;
    }    
    /*
    ...
    protected function _waterfall($mod,$where='',$order = "",$pagesize=5) {        
        ...
        //file_put_contents("./data/debug.log",$_SERVER["REQUEST_URI"]."\r\n".print_r($mod->getLastSql(),true)."\r\n",FILE_APPEND);
